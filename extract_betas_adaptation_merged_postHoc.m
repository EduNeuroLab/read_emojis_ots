%This script performs posthoc tests on the repeated measure ANOVA 
%for experiment 2 (Factors: repetition, stimulus)
...
rois={'rh_CC_fLOC3_color_vs_all'}';
    ...
    %create plot
    caption_x=['TR'; 'TN';'  ';'ER'; 'EN';'  '; 'MR'; 'MN'];
    caption_y='betas';
    fig=mybar(xvalues,xerror, caption_x,[],[0 0 0.5; 0 .6 1; 0 0 0; 0 0.5 0; 0.6 1 0; 0 0 0; 0.4 0 0.4; 1 0.2 1;0 0 0;  1 0.4 0; 1 0.8 0],2);
    ...
    table_for_rmANOVA=table(reshaped_betas_inverted(:,1),reshaped_betas_inverted(:,2),...
        reshaped_betas_inverted(:,3),reshaped_betas_inverted(:,4),reshaped_betas_inverted(:,5),reshaped_betas_inverted(:,6),...
        'VariableNames',{'tr','tn','er','en','mr','mn'});
    WithinDesign = table([1 1 2 2 3 3]',[1 2 1 2 1 2]','VariableNames',{'stim','rep'});
    WithinDesign.stim = categorical(WithinDesign.stim);
    WithinDesign.rep = categorical(WithinDesign.rep);
    
    rm = fitrm(table_for_rmANOVA, 'tr,tn,er,en,mr,mn~1','WithinDesign',WithinDesign);
    ranovatable = ranova(rm,'WithinModel','stim*rep')
    %run tukey-kramer post hoc test
    multcompare(rm, 'rep','by','stim','ComparisonType',['tukey-kramer'])
