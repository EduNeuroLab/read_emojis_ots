...
function plot_corr_avg(cavg, csym, OutDir, outfilename, labelstring, session, roi, visible);
         % plot correlation
        ...
        subplot(1,2,1); imagesc(cavg, [-1.0 1.0]); axis('image'); cmap=mrvColorMaps('coolhot'); colormap(cmap); colorbar; title('cavg')
        set(gca,'Xtick', [1:1:numconds], 'XtickLabel',labelstring)
        set(gca,'Ytick', [1:1:numconds], 'YtickLabel',labelstring)
        subplot(1,2,2); imagesc(csym, [-1.0 1.0]); axis('image');cmap=mrvColorMaps('coolhot'); colormap(cmap);colorbar; title('csym')
        set(gca,'Xtick', [1:1:numconds], 'XtickLabel',labelstring)
        set(gca,'Ytick', [1:1:numconds], 'YtickLabel',labelstring)
