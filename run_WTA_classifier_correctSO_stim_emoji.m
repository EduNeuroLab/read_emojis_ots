function WTA_correctSO_stim_emoji=run_WTA_classifier_correctSO_stim_emoji(runs,numconds,one_run_out_corr_all);
...
for m=1:length(runs)
    ...
    for w=1:numconds
        ...
        correct_matrix=[1 3; 2 4; 1 3; 2 4;]; %first we will create a matrix saying which 
        ...
        correctSO_final(1,m)=(correctSO(1,m)+correctSO(3,m)) % finally we add  up those conditions that can be confused
        correctSO_final(2,m)=(correctSO(2,m)+correctSO(4,m))
    end
end

WTA_correctSO_stim_emoji=mean(correctSO_final,2);
end